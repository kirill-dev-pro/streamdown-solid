...
export const ImageComponent = (props: ImageComponentProps) => {
  const { node, className, src, alt, ...rest } = props
  const downloadImage = async () => {
    ...
    try {
      ...
      const hasExtension =
        originalFilename.includes('.') && originalFilename.split('.').pop()?.length! <= 4
  ...
  return (
    <div class='group relative my-4 inline-block' data-streamdown='image-wrapper'>
      ...
      <img
        alt={alt}
        class={cn('max-w-full rounded-lg', className)}
        data-streamdown='image'
        src={src}
        {...rest}
      />
      <div class='pointer-events-none absolute inset-0 hidden rounded-lg bg-black/10 group-hover:block' />
      <button
        class={cn(
